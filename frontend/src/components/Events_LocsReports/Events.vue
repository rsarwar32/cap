				...
				<table class="table table-striped">
					...
					<tbody>
						<tr v-for="x in rows" :key="x">
							...
							<td>{{ x.location.location_address }}</td>
							<td>{{ x.location.location_city }}</td>
							<td>{{ x.location.location_state }}</td>
							<td>{{ x.location.location_zip }}</td>
							<td>
...
export default {
	...
	methods: {
		deleteEvents(id) {
			let apiURL = `http://localhost:3000/api/eventDelete/${id}`;
			console.log(id);
